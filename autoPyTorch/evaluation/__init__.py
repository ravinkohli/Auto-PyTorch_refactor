...
import autoPyTorch.evaluation.utils
import autoPyTorch.utils.logging_
...
class ExecuteTaFuncWithQueue(AbstractTAFunc):
    ...
    def __init__(self, backend, autoPyTorch_seed, resampling_strategy, metric,
        ...
        self.autoPyTorch_seed = autoPyTorch_seed
        self.resampling_strategy = resampling_strategy
        self.initial_num_run = initial_num_run
        # TODO: change to metrics
        self.metric = metric
        self.resampling_strategy = resampling_strategy
        self.resampling_strategy_args = resampling_strategy_args
        # TODO: no need for this as get_metrics handles this, maybe change to all_supported_metrics
    ...
    def run(
        ...
        arguments = dict(
            logger=autoPyTorch.util.logging_.get_logger("pynisher"),
        ...
        if obj.exit_status in (pynisher.TimeoutException, pynisher.MemorylimitException):
            ...
            try:
                info = autoPyTorch.evaluation.util.read_queue(queue)
                result = info[-1]['loss']
        ...
        if (
            ...
            learning_curve = autoPyTorch.evaluation.util.extract_learning_curve(info)
            learning_curve_runtime = autoPyTorch.evaluation.util.extract_learning_curve(
                info, 'duration'
            )
            ...
            train_learning_curve = autoPyTorch.evaluation.util.extract_learning_curve(
                info, 'train_loss'
            )
            ...
            if self._get_validation_loss:
                validation_learning_curve = autoPyTorch.evaluation.util.extract_learning_curve(
            ...
            if self._get_test_loss:
                test_learning_curve = autoPyTorch.evaluation.util.extract_learning_curve(
                    info, 'test_loss',
                )
        ...
        autoPyTorch.evaluation.util.empty_queue(queue)
        self.logger.debug(
